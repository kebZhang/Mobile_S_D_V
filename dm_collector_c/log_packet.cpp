...
{
    FILE *debug_file_2 = fopen("ty_debug1.txt","a");

    //fprintf(debug_file_2,"debug file 2 open in on_demand_dcode function\n");
    
    //fprintf(debug_file, "type_id=%llx\n",type_id);
    int offset = 0;
    switch (type_id) {
        ...
        case LTE_RRC_OTA_Packet:
            fprintf(debug_file_2,"rrc decode func\n");
...
void print_hex(const char *b, size_t length)
{
    printf("data at point b: \n");
...
decode_log_packet(const char *b, size_t length, bool skip_decoding) {
    ...
    //ty debug add
    FILE *debug_file1 = fopen("ty_debug.txt","a");
    fprintf(debug_file1,"debug file 1 open in decode_log_packet\n");
    fprintf(debug_file1,"pointer b \n");
    for(size_t i=0;i<length;i++)
    {
        fprintf(debug_file1,"%02x",(unsigned char)b[i]);
    }
    fprintf(debug_file1,"\n pointer b end \n");
    ...
    ////ty debug metadata header
    // fprintf(debug_file1,"after decode header\n");
    // int type_id1 = (int) _map_result_field_to_name(
    //     result,
    //     "type_id",
    //     LogPacketTypeID_To_Name,
    //     ARRAY_SIZE(LogPacketTypeID_To_Name, ValueName),
    //     "Unsupported");
    // fprintf(debug_file1,"type id is 0x%02x\n", type_id1);

    // int log_msg_len_1 = (int) _map_result_field_to_name(
    //     result,
    //     "log_msg_len",
    //     LogPacketTypeID_To_Name,
    //     ARRAY_SIZE(LogPacketTypeID_To_Name, ValueName),
    //     "Unsupported");    
    // fprintf(debug_file1,"log_msg_len is 0x%02x\n", log_msg_len_1);

    // int timestamp_1 = (int) _map_result_field_to_name(
    //     result,
    //     "timestamp",
    //     LogPacketTypeID_To_Name,
    //     ARRAY_SIZE(LogPacketTypeID_To_Name, ValueName),
    //     "Unsupported");    
    // fprintf(debug_file1,"timestamp is %04x\n", timestamp_1);
    ...
    for(size_t i=0;i<ARRAY_SIZE(LogPacketTypeID_To_Name,ValueName);i++)
    {
        if(LogPacketTypeID_To_Name[i].val == type_id)
        {
            //type_id = LogPacketTypeID_To_Name[i].val;
            fprintf(debug_file1," this is a %x and %s\n", LogPacketTypeID_To_Name[i].val, LogPacketTypeID_To_Name[i].name);
        }
    }
    
    fprintf(debug_file1,"debug file1 close\n");
    fclose(debug_file1);
