...
static PyObject *dm_collector_c_receive_log_packet_debug(PyObject *self, PyObject *args);
...
static PyMethodDef DmCollectorCMethods[] = {
	...
	{"set_sampling_rate",   dm_collector_c_set_sampling_rate,   METH_VARARGS,
        ...
        {"receive_log_packet_debug", dm_collector_c_receive_log_packet_debug, METH_VARARGS,
                                                                        "Debug function to process hardcoded log packet data.\n"
                                                                        "\n"
                                                                        "Returns:\n"
                                                                        "    Decoded log packet.\n"
        },
...
generate_log_config_msgs(PyObject *file_or_serial, PyObject *type_names) {
    ...
    FILE *debug_file_4 = fopen("ty_debug_4.txt","a");
    fprintf(debug_file_4,"debug file 4 open\n");
    fprintf(debug_file_4, "success is %d\n",success);
    //fclose(debug_file_4);
    ...
    for (size_t i = 0; i < type_id_vectors.size(); i++) {
        ...
        for(int id : v)
        {
            fprintf(debug_file_4, "logcode = %02x\n", id);
        }
        buf = encode_log_config(SET_MASK, v);
        if (buf.first != NULL && buf.second != 0) {
            fprintf(debug_file_4,"buffer content %zu\n", buf.second);
            for(size_t j=0;j<buf.second;j++)
            {
                fprintf(debug_file_4,"%02x", buf.first[j]);
            }
            fclose(debug_file_4);            
            (void) send_msg(file_or_serial, buf.first, buf.second);
            delete[] buf.first;
...
// Return: decoded_list or None
static PyObject *
dm_collector_c_receive_log_packet_debug(PyObject *self, PyObject *args) {
    unsigned char debug_data[] = {0x00,0x00,0x00,0x00,0x30,0x00,0x6c,0xb1,0x1c,0x0f,0xe1,0xb0,0x1a,0x92,0x02,0x01,0x31,0xbc,0x00,0x00,0xf7,0x40,0x00,0x00,0xa8,0x80,0x60,0x45,0x0a,0x18,0x16,0x26,0xfc,0x16,0x00,0x00,0x00,0x00,0x00,0x00,0xf6,0x24,0x02,0x00,0xb0,0x06,0x01,0x00,0x00,0x00,0x00,0x00};
    std::string debug_string;
    std::string a;
    for(unsigned char byte : debug_data)
    {
        debug_string.push_back(static_cast<char>(byte));
    }
    const char *s = debug_string.c_str();
    FILE *debug_file = fopen("ty_debug.txt","a");
    fprintf(debug_file,"debug file open\n");
    for(size_t i=0;i<debug_string.size();i++)
    {
