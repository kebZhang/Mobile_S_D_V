...
#define DIAG_IOCTL_BUF_DRAIN_START		8888
#define DIAG_IOCTL_BUF_DRAIN_END		9999
...
static int mode_buffering = DIAG_BUFFERING_MODE_STREAMING;
...
enable_logging (int fd, int mode)
{
	...
	*/
	...
	buffering_mode.mode = mode_buffering;
	...
	FILE *debug_file_enable = fopen("Ty_debug_diag_revealer.txt","a");

    if ((fp = popen(board_pf_cmd, "r")) != NULL) {
        while (fgets(board_name, 256, fp) != NULL) 
		{
			fprintf(debug_file_enable, "board platform: %s\n", board_name);
        	// printf("OUTPUT: %s\n", board_name);
    	}
    	pclose(fp);
    }
	...
	{
		fprintf(debug_file_enable, "board platform: else\n");
	}
    ...
    if ((fp = popen(sys_ver_cmd, "r")) != NULL) {
        while (fgets(system_version, 256, fp) != NULL) {
			fprintf(debug_file_enable, "system_version: %s\n", system_version);
        	// printf("OUTPUT: %s\n", system_version);
    	}
    	pclose(fp);
    }
    ...
    if (strstr(board_name, "lito") != NULL && strstr(system_version, "11") != NULL){
		...
		//ty debug
		debug_file_enable = fopen("Ty_debug_diag_revealer.txt","a+");
		fprintf(debug_file_enable, "recognized lito and android 11\n");
		...
		fprintf(debug_file_enable, "DIAG_IOCTL_QUERY_CON_ALL ret = %d\n", ret);
		...
		fprintf(debug_file_enable, "DIAG_IOCTL_SWITCH_LOGGING ret = %d\n", ret);
		fclose(debug_file_enable);
		goto next;
    }
	// Testing end
...
main (int argc, char **argv)
{
	...
	FILE *debug_file;
	debug_file = fopen("Ty_debug_diag_revealer.txt","w+");
	fprintf(debug_file, "open Ty_debug_diag_revealer\n");
	fclose(debug_file);
	debug_file = fopen("Ty_debug_diag_revealer.txt","a+");
	...
	double ts_main = get_posix_timestamp();
	fprintf(log_file, "Diag revealer start at %lf\n", ts_main);
	...
	//generate_cfg
	int logcode_num=1;
	uint16_t logcode_list[1] = {0xB16C};
	...
	// Read config file
	BinaryBuffer buf_write = read_diag_cfg(filename_diag_cfg);
	if (buf_write.p == NULL || buf_write.len == 0) {
		return -8001;
	}
	fprintf(debug_file, "after read cfg\n");

	if(strcmp(argv[1], "streaming") == 0){
		mode_buffering = DIAG_BUFFERING_MODE_STREAMING;
	}else if(strcmp(argv[1], "circular") == 0){
		mode_buffering = DIAG_BUFFERING_MODE_CIRCULAR;
	}else if(strcmp(argv[1], "threshold") == 0){
		mode_buffering = DIAG_BUFFERING_MODE_THRESHOLD;
	}else{
		printf("Diag_revealer: wrong buffering mode input\n");
		return 0;
	}
	fprintf(debug_file, "set buffering mode to %d\n", mode_buffering);
	...
	if (fd < 0) 
	{
		fprintf(debug_file, "can not open dev diag port \n");
		perror("open diag dev");
		return -8002;
	}
	fprintf(debug_file, "after open port\n");
	...
	fprintf(debug_file, "after enable logging\n");
	...
	fprintf(debug_file, "after write cmd\n");
	...
	// ***************************** drain in kernel ****************************
	// uint8_t peripheral = 0;
	// ret = ioctl(fd, DIAG_IOCTL_BUF_DRAIN_START, &peripheral);
	// if(ret < 0){
	// 	fprintf(log_file, "ioctl DIAG_IOCTL_BUF_DRAIN_START fails, ret= %d\n", ret);
	// }else{
	...
	while (1) 
	{
		...
		// if(ret_drain < 0){
		// 	fprintf(log_file, "ioctl DIAG_IOCTL_PERIPHERAL_BUF_DRAIN fails, ret= %d\n", ret_drain);
		// }else{
		...
		if (read_len > 0) 
		{
			if (*((int *)buf_read) == USER_SPACE_DATA_TYPE) 
			{
				...
				for (i = 0; i < num_data; i++) 
				{
					...
					// ret_err = fprintf(logfile,"timestamp :%lf\n", ts);
					// if (ret_err < 0) {
					// 	// LOGI("Pipe closed, diag_revealer will exit");
					...
					// ret_err = fwrite(buf_read + offset + 4, sizeof(char), msg_len, logfile);
	...
	*/
	...
	// }else{
	// 	fprintf(log_file, "ioctl DIAG_IOCTL_BUF_DRAIN_END succeeds, ret= %d\n", ret);
	// }
