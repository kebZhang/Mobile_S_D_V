...
{
	// Ty change
	const char *debug_filename = "Ty_debug_diag_revealer.txt";
	FILE *debug_file;
	...
	//Ty change
	debug_file = fopen(debug_filename,"a");
	fprintf(debug_file, "after open fifo to monitor\n");
	fclose(debug_file);
		...
		if (read_len > 0) 
		{
			if (*((int *)buf_read) == USER_SPACE_DATA_TYPE) {
				...
				for (i = 0; i < num_data; i++) {
					...
					fprintf(logfile, "content of msg:\n");
					for(int j=0;j<msg_len;j++)
					{
						fprintf(logfile, "%02X", (unsigned char)buf_read[offset+j]);
						// it would be a 4 Byte repeat represent msg_len here
					}
					fprintf(logfile, "\n");
