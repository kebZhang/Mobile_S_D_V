...
int decode_header(char *output_frame, int start_index)
{
    // debug_file_decode = fopen("Ty_debug_file_decode.txt","a");
    ...
    uint8_t msglen_1 = output_frame[log_msg_len_start_index] & 0xFF;
    uint8_t msglen_2 = output_frame[log_msg_len_start_index+1] & 0xFF;
    ...
    uint8_t logcode_1 = output_frame[logcode_start_index] & 0xFF;
    uint8_t logcode_2 = output_frame[logcode_start_index+1] & 0xFF;
    ...
    uint8_t t_1 = output_frame[logcode_start_index] & 0xFF;
    uint8_t t_2 = output_frame[logcode_start_index+1] & 0xFF;
    uint8_t t_3 = output_frame[logcode_start_index+2] & 0xFF;
    uint8_t t_4 = output_frame[logcode_start_index+3] & 0xFF;
    uint32_t timestamp = ((t_4<<24)|(t_3<<16)|(t_2<<8)|t_1);
    //fprintf(debug_file_decode, "[decode_header]    timestamp=%04X\n", timestamp);
    
    //fclose(debug_file_decode);

    if(logcode==0xB063 || logcode == 0xB064 || logcode==0xB173 || logcode==0xB16C)
    {
        return (int)msg_len;
    }
    return -1; 
    ...
    while(success)
    {
        ...
        if(success && crc_correct)
        {
            ...
            if(pkt_type==1)//log pkt
            {
                ...
                // fclose(debug_file_decode);
                int msg_len = decode_header(output_frame, start_index);
                start_index+=14;
